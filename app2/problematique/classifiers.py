...
def compute_prob_dens_gaussian(train_data, test_data1, test_data2):
    ...
    for i in range(x):  # itère sur toutes les classes
        ...
        # DONEDO L2.E2.3 Compléter le calcul ici
        mahalanobis1 = np.array(
            [
                np.dot(
                    np.dot((test_data1[j] - mean_list[i]), inv_cov_list[i]),
                    (test_data1[j] - mean_list[i]).T,
                )
                for j in range(t1)
            ]
        )
        ...
        mahalanobis2 = np.array(
            [
                np.dot(
                    np.dot((test_data2[j] - mean_list[i]), inv_cov_list[i]),
                    (test_data2[j] - mean_list[i]).T,
                )
                for j in range(t2)
            ]
        )
...
def ppv_classify(n_neighbors, train_data, classes, test1, test2=None):
    ...
    # DONEDO L2.E3.1 Compléter la logique pour utiliser la librairie ici
    ...
    # entraine le classificateur
    kNN.fit(train_data, classes)
    # prédit les classes pour les données de test1
    predictions_test1 = kNN.predict(test1)
    # prédit les classes pour les données de test2 si présentes
    if np.asarray(test2).any():
        predictions_test2 = kNN.predict(test2)
...
def kmean_alg(n_clusters, data):
    ...
    for i in range(x):
        # DONEDO L2.E3.3 compléter la logique pour utiliser la librairie ici
        kmeans_C = km(n_clusters, random_state=0).fit(data[i])
